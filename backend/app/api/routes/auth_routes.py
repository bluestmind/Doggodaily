...
from ...auth.utils import TokenManager, password_validator, SecurityUtils
...
def login():
    ...
    try:
        ...
        if suspicious_activity.get('is_suspicious', False):
            response_data['security_alert'] = {
                'type': suspicious_activity.get('risk_level', 'medium'),
                'indicators': suspicious_activity.get('indicators', []),
                'message': 'Unusual login activity detected',
                'require_verification': suspicious_activity.get('risk_level') == 'high'
            }
...
# Google OAuth (Authlib) for session-based login
@auth_bp.route('/google/login', methods=['GET'])
def google_login():
    try:
        # Ensure client configured at app startup
        ...
        logger.error(f"Google login init error: {str(e)}")
        return jsonify({'success': False, 'message': 'Failed to start Google login'}), 500
...
def google_callback():
    try:
        ...
        if not userinfo:
            return jsonify({'success': False, 'message': 'Failed to fetch Google user info'}), 400

        email = (userinfo.get('email') or '').lower().strip()
        name = userinfo.get('name') or (email.split('@')[0] if email else 'Google User')
        if not email:
            return jsonify({'success': False, 'message': 'Email permission is required'}), 400
        ...
        TokenManager.log_security_event(user.id, 'successful_login', 'Google OAuth')
...
# Current user profile (session-based)
...
def logout():
    try:
        ...
        # Build response and explicitly clear cookies
        ...
        # Expire the remember cookie if present
...
def logout_all():
    try:
        # If tracking sessions, end all here. For now, just logout current.
        uid = current_user.id
        logout_user()
...
def get_sessions():
    try:
        # Optional: return sessions if you track them; return empty list otherwise
...
def end_session(session_id):
    try:
        # Optional: implement if tracking sessions
...
def change_password():
    ...
    try:
        ...
        # Optionally end other sessions if you track them
